...
<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        ...
        <div class="flex gap-3 mt-4 md:mt-0">
            <button id="btnExportPDF" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200 flex items-center">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Laba</th>
                    </tr>
                </thead>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadReportData() {
        ...
        try {
            // In a real app, you would fetch from API with outlet ID
            // Example: /api/profit-loss/outlet/${outletId}?start_date=${startDate}&end_date=${endDate}
            console.log(`Loading report for outlet ${outletId} from ${startDate} to ${endDate}`);
    ...
    function generateMockReportData(startDate, endDate, outletId) {
        // Generate random data for demo purposes based on outlet
        ...
        return {
            ...
            transactions: generateMockTransactions(startDate, endDate, outletId)
        };
    }

    function generateMockTransactions(startDate, endDate, outletId) {
        const transactions = [];
        const start = new Date(startDate);
        const end = new Date(endDate);
        const daysDiff = Math.ceil((end - start) / (1000 * 60 * 60 * 24));
        
        for (let i = 0; i < Math.min(daysDiff * 5, 20); i++) {
            const randomDay = new Date(start);
            randomDay.setDate(start.getDate() + Math.floor(Math.random() * (daysDiff + 1)));
            
            const types = ['Minuman', 'Makanan', 'Lainnya'];
            const type = types[Math.floor(Math.random() * types.length)];
            
            const baseAmount = parseInt(outletId) * 10000;
            const pendapatan = Math.floor(Math.random() * 50000) + 10000 + baseAmount;
            const biayaBahan = Math.floor(pendapatan * (0.2 + Math.random() * 0.3));
            const laba = pendapatan - biayaBahan;
            
            transactions.push({
                date: randomDay.toISOString().split('T')[0],
                type: type,
                description: `Penjualan ${type.toLowerCase()} ${i + 1} - Outlet ${outletId}`,
                revenue: pendapatan,
                cost: biayaBahan,
                profit: laba
            });
    ...
    function updateReportUI(data) {
        ...
        const margin = Math.min(data.laba.margin, 100);
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            transaction.type === 'Minuman' ? 'bg-green-100 text-green-800' :
                            transaction.type === 'Makanan' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${formatRupiah(transaction.revenue)}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${formatRupiah(transaction.cost)}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium ${profitClass}">
                        ${profitSign}${formatRupiah(transaction.profit)}
                    </td>
                </tr>
            `;
        });
