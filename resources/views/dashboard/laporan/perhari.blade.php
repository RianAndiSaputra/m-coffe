...
<script>
        ...
        currentData.orders.forEach(order => {
            ...
            row.innerHTML = `
                ...
                <td class="py-4 text-right">
                    <button onclick="showTransactionDetail('${order.id}')" class="text-[#3b6b0d] hover:text-[#335e0c]">
    ...
    function showAlert(type, message) {
        ...
        alert.className = `px-4 py-3 rounded-lg shadow-md ${type === 'error' ? 'bg-red-100 text-red-700' : 
                        type === 'success' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`;
        alert.innerHTML = `
            <div class="flex items-center justify-between">
